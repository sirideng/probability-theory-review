...
import { numpyReferenceCategories, numpyReferenceItems } from '../data/numpy-reference'

function scrollToCategory(sectionId: string) {
...
export default function NumpyReferencePage() {
  return <LongPageNavigation tableOfContents={false}><div className="page-container pb-24 pt-8 sm:pt-14">
    ...
    <section className="liquid-content-card mt-8 rounded-[24px] border p-5 sm:p-7">
      <div className="flex items-start gap-4"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-[14px] bg-violet-500/10 text-violet-600"><Layers3 size={20} /></span><div><h2 className="text-[19px] font-semibold">当前收录范围</h2><p className="mt-2 text-[14px] leading-7 text-black/55">当前收录 {numpyReferenceItems.length} 组机器学习高频 NumPy 方法，覆盖数组与形状、数据处理、数值函数和线性代数。这是辅助速查页，不改变视频 1—36 的课程范围，不计入 14 个学习模块的完成进度。</p></div></div>
      <nav aria-label="NumPy 分类导航" className="mt-6 flex flex-wrap gap-2 border-t border-black/[0.06] pt-5">
        {numpyReferenceCategories.map((category) => <button key={category.id} type="button" aria-controls={category.id} onClick={() => scrollToCategory(category.id)} className="min-h-10 rounded-full border border-black/[0.06] bg-black/[0.035] px-4 text-[13px] font-semibold text-black/60 transition hover:bg-blue/[0.07] hover:text-blue focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue/15">{category.label}</button>)}
      </nav>
    </section>

    <div className="mt-12 space-y-16">
      {numpyReferenceCategories.map((category) => {
        const categoryItems = numpyReferenceItems.filter((item) => item.category === category.label)
        return <section key={category.id} id={category.id} className="scroll-mt-24">
          <div className="mb-6 flex items-end justify-between gap-4 border-b border-black/[0.07] pb-4">
            <h2 className="text-[28px] font-semibold tracking-[-0.035em] sm:text-[32px]">{category.label}</h2>
            <span className="text-[12px] font-medium text-black/45">{categoryItems.length} 张卡片</span>
          </div>
          <div className="grid min-w-0 gap-6 xl:grid-cols-2">
            {categoryItems.map((item) => <article key={item.id} className="liquid-content-card min-w-0 overflow-hidden rounded-[26px] border">
              <div className="p-5 sm:p-7">
                ...
                <h3 className="mt-5 text-[24px] font-semibold tracking-[-0.03em]">{item.title}</h3>
              ...
              <div className="p-5 sm:p-7">
                ...
                <div className="mt-3 flex min-w-0 flex-wrap items-start gap-3">
                  {item.outputs.map((output, outputIndex) => <pre key={`${item.id}-output-${outputIndex}`} aria-label={`输出 ${outputIndex + 1}`} className="w-fit max-w-full overflow-x-auto whitespace-pre rounded-[14px] bg-black/[0.035] px-4 py-3 font-mono text-[14px] leading-7 text-black/72">{output}</pre>)}
                </div>
