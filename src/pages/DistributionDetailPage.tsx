...
function RangeControl({ control, value, onChange, color }: { control: ParamConfig; value: number; onChange: (value: number) => void; color: string }) {
  ...
  return <label className="block"><div className="flex items-center justify-between gap-4"><span className="text-[15px] font-semibold">{control.label} <span className="ml-1 font-serif text-black/35">{control.symbol}</span></span><output className="min-w-[56px] rounded-lg px-2.5 py-1 text-center font-mono text-sm font-semibold" style={{ color, backgroundColor: `${color}12` }}>{Number.isInteger(control.step) ? value : value.toFixed(2).replace(/0$/, '')}</output></div><input type="range" min={control.min} max={control.max} step={control.step} value={value} onChange={(event) => onChange(Number(event.target.value))} className="range-slider mt-4 w-full" style={{ '--progress': `${progress}%`, '--range-color': color } as React.CSSProperties}/></label>
}
...
function DistributionDetailContent({ slug }: { slug: string }) {
  ...
  return <div className="page-container pb-28 pt-8 sm:pt-14">
    ...
    {distribution.standardizationExample && <section className="liquid-content-card mt-6 min-w-0 rounded-[28px] border p-6 sm:p-8"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="flex items-center gap-3"><span className="section-number" style={{ color: distribution.color, backgroundColor: `${distribution.color}12` }}>02</span><div className="min-w-0"><p className="text-[11px] font-semibold uppercase tracking-[0.13em] text-black/35">标准化解题法</p><h2 className="mt-1 text-2xl font-semibold">{distribution.standardizationExample.title}</h2></div></div><span className="self-start rounded-full bg-violet-50 px-3 py-1.5 text-xs font-semibold text-violet-600">{distribution.standardizationExample.source}</span></div><div className="mt-7 rounded-[20px] border border-black/[0.05] bg-white/60 p-5 sm:p-6"><p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-black/35">题目</p><p className="mt-3 text-[16px] leading-8 text-black/70 sm:text-[17px]">{distribution.standardizationExample.problem}</p></div><div className="mt-5 rounded-[18px] bg-violet-50/70 p-5"><p className="text-sm font-semibold text-violet-700">入手方法</p><p className="mt-2 text-[15px] leading-7 text-black/60">{distribution.standardizationExample.method}</p></div><div className="mt-7 space-y-5">{distribution.standardizationExample.steps.map((step, index) => <div key={step.text} className="grid min-w-0 gap-3 sm:grid-cols-[36px_1fr]"><span className="grid h-8 w-8 place-items-center rounded-full bg-black/[0.045] text-xs font-semibold text-black/45">{index + 1}</span><div className="min-w-0"><p className="text-[15px] leading-7 text-black/60">{step.text}</p>{step.formula && <MathFormula value={step.formula} block className="property-formula mt-3 w-full"/>}</div></div>)}</div><div className="mt-7 min-w-0 overflow-hidden rounded-[20px] border border-green-600/10 bg-green-50/70 p-5 sm:p-6"><p className="text-sm font-semibold text-green-700">结论</p><p className="mt-2 text-[15px] leading-7 text-black/60">{distribution.standardizationExample.answer}</p><MathFormula value={distribution.standardizationExample.answerFormula} block className="mt-4 max-w-full overflow-x-auto text-xl text-green-700"/></div></section>}

    <section className="mt-6 grid gap-5 lg:grid-cols-3"><div className="knowledge-section"><div className="flex items-center gap-2"><BookOpenCheck size={19} className="text-green-600"/><h2 className="text-xl font-semibold">核心性质</h2></div><ul className="mt-5 space-y-4">{distribution.properties.map((item)=><li key={item} className="flex gap-3 text-[15px] leading-7 text-black/60"><Check size={16} className="mt-1 shrink-0 text-green-600"/><span>{item}</span></li>)}</ul></div><div className="knowledge-section"><div className="flex items-center gap-2"><Sparkles size={19} className="text-blue"/><h2 className="text-xl font-semibold">考试重点</h2></div><ul className="mt-5 space-y-4">{distribution.examTips.map((item)=><li key={item} className="flex gap-3 text-[15px] leading-7 text-black/60"><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-blue"/><span>{item}</span></li>)}</ul></div><div className="knowledge-section"><div className="flex items-center gap-2"><AlertTriangle size={19} className="text-rose-500"/><h2 className="text-xl font-semibold">易错点</h2></div><ul className="mt-5 space-y-4">{distribution.mistakes.map((item)=><li key={item} className="flex gap-3 text-[15px] leading-7 text-black/60"><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-rose-500"/><span>{item}</span></li>)}</ul></div></section>
  </div>
}
