    ...
    <div className="page-container-narrow pb-28 pt-8 sm:pt-14">
      ...
      <header className="mt-10 border-b border-black/[0.075] pb-12">
        ...
        <div className="mt-4 flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex items-center gap-4 text-sm text-black/65"><span className="inline-flex items-center gap-1.5"><Clock3 size={15} />{point.readTime} 分钟</span><span>更新于 {point.updatedAt}</span></div>
        </div>
      ...
      <div className="mt-8 grid gap-5">
        ...
        <section className="knowledge-section">
          ...
          {enhancement && <div className="mt-7 divide-y divide-black/[0.07] border-t border-black/[0.07]">{enhancement.problemTypes.map((item) => <article key={item.title} className="py-6"><h3 className="text-[19px] font-semibold tracking-tight">{item.title}</h3><dl className="mt-4 grid gap-3 text-[15px] leading-7 sm:grid-cols-[88px_1fr]"><dt className="font-semibold text-black/60">题型特点</dt><dd className="text-black/60">{item.features}</dd><dt className="font-semibold text-black/60">解题方法</dt><dd className="text-black/60">{item.method}</dd><dt className="font-semibold text-black/60">关键步骤</dt><dd className="flex flex-wrap gap-2">{item.steps.map((step, index) => <span key={step} className="rounded-full bg-black/[0.045] px-3 py-1 text-[13px] text-black/65">{index + 1}. {step}</span>)}</dd></dl></article>)}</div>}
        </section>
            ...
            {exerciseGroup.exercises.map((exercise, index) => <article key={exercise.id} className="overflow-hidden rounded-[20px] border border-black/[0.065] bg-[#fbfbfd]">
              <div className="border-b border-black/[0.06] bg-white px-5 py-5 sm:px-7">
                <div className="flex flex-wrap items-center gap-2.5">
                  ...
                  <span className={`ml-auto rounded-full px-2.5 py-1 text-xs font-semibold ${exercise.difficulty === '强化' ? 'bg-orange-50 text-orange-700' : 'bg-green-50 text-green-700'}`}>{exercise.difficulty}</span>
                </div>
                <p className="mt-2 text-xs font-medium leading-5 text-black/65">{exercise.source}</p>
              ...
              <div className="px-5 py-5 sm:px-7">
                <dl className="grid gap-3 text-[15px] leading-7 sm:grid-cols-[72px_1fr]">
                  <dt className="font-semibold text-black/60">考查重点</dt><dd className="text-black/60">{exercise.focus}</dd>
                  <dt className="font-semibold text-black/60">入手方法</dt><dd className="text-black/60">{exercise.method}</dd>
                </dl>
