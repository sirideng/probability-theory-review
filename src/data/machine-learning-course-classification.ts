...
export const classificationLessons: MachineLearningLesson[] = [
  {
    ...
    intuition: [
      '二分类通常把两个类别记作 y=0 和 y=1。模型先计算线性得分 z=\\mathbf w^{\\mathsf T}\\mathbf x+b，再通过 Sigmoid 函数（逻辑函数）得到正类概率估计 \\hat p。',
    ...
    formulas: [
      { label: '线性得分', formula: 'z=\\mathbf w^{\\mathsf T}\\mathbf x+b', explanation: 'z 是 Sigmoid 函数之前的实数线性得分。', symbols: [{ symbol: '\\mathbf x', meaning: '输入特征向量' }, { symbol: '\\mathbf w', meaning: '权重向量' }, { symbol: 'b', meaning: '偏置' }] },
      { label: 'Sigmoid 函数（逻辑函数）', formula: '\\hat p=\\sigma(z)=\\frac{1}{1+e^{-z}},\\qquad 0<\\hat p<1', explanation: '在逻辑回归模型假设下，\\hat p 用于估计 P(y=1\\mid\\mathbf x)。', symbols: [{ symbol: 'z', meaning: '线性得分' }, { symbol: '\\hat p', meaning: '正类概率估计' }, { symbol: 'e', meaning: '自然常数' }] },
    ...
    algorithm: [
      ...
      { title: '得到概率估计', detail: '将线性得分送入 Sigmoid 函数，得到正类概率估计 \\hat p。' },
    ...
    example: {
      ...
      steps: [
        { text: '把线性得分代入 Sigmoid 函数。', formula: '\\hat p=\\sigma(\\ln4)=\\frac{1}{1+e^{-\\ln4}}=\\frac{1}{1+1/4}=0.8' },
...
print(prediction)`,
  ...
  },
  {
    ...
    problem: '将 Sigmoid 预测与平方误差组合时，所得关于参数的目标函数通常不再具有二元交叉熵逻辑回归所具有的良好凸优化结构，因此本课程采用二元交叉熵作为损失函数。',
    intuition: [
      ...
      '梯度公式的代数结构与线性回归相似，但逻辑回归中的 \\hat p 是 Sigmoid 概率估计，J 是平均二元交叉熵代价。',
    ],
    formulas: [
...
print(round(binary_loss(0.1, 1), 4))`,
    ...
    misconceptions: [
      { wrong: '逻辑回归继续使用线性回归的平方误差即可得到同样的优化结构。', why: 'Sigmoid 预测与平方误差组合时，所得关于参数的目标函数通常不再具有二元交叉熵逻辑回归的良好凸结构。', correct: '当前二分类逻辑回归使用二元交叉熵损失。' },
      { wrong: '梯度代数形式相似，所以线性回归与逻辑回归完全相同。', why: '二者的预测函数、单样本损失和训练集代价不同。', correct: '线性回归使用实数预测值；逻辑回归使用 \\hat p 这一 Sigmoid 概率估计。' },
  ...
  },
  {
    ...
    videoRange: '视频 32—34',
    summary: '过拟合的核心表现是训练集拟合很好，但验证误差较高；L2 正则化通过惩罚较大权重来调节拟合与复杂度。',
    problem: '高次多项式可以把训练误差压得很低，却可能在样本之间剧烈摆动。训练集用于拟合模型参数；验证集用于模型比较、超参数选择和过拟合诊断；测试集只用于模型确定后的最终评估，不参与反复调参。',
    intuition: [
      ...
      '增加训练数据、移除无关或高度冗余的特征、加入正则化都可能缓解过拟合；机械减少特征也可能丢失有效信息。当前课程重点是 L2 正则化。',
      'L2 正则化把数据拟合项与权重平方惩罚共同组成完整目标函数。较大权重受到更强惩罚，但最终参数仍同时受到数据拟合项影响。',
      'λ=0 表示不正则化；适当 λ 可能改善泛化；λ 过大可能使模型过于简单而欠拟合。本课程约定偏置 b 不加入正则项。',
    ],
    formulas: [
      { label: 'L2 正则化目标函数', formula: 'J_{\\mathrm{reg}}(\\mathbf w,b)=J_{\\mathrm{data}}(\\mathbf w,b)+\\frac{\\lambda}{2m}\\sum_{j=1}^{n}w_j^2', explanation: 'J_data 是数据拟合部分，权重平方和是正则项，二者共同组成完整正则化目标函数 J_reg。', symbols: [{ symbol: 'J_{\\mathrm{data}}', meaning: '模型原有的数据拟合代价' }, { symbol: 'J_{\\mathrm{reg}}', meaning: '完整正则化目标函数' }, { symbol: '\\lambda\\ge0', meaning: '正则化强度超参数' }, { symbol: 'n', meaning: '特征数量' }, { symbol: 'b', meaning: '本课程约定不加入正则项的偏置' }] },
    ...
    algorithm: [
      { title: '识别拟合状态', detail: '结合训练误差与验证误差判断欠拟合、合适拟合或过拟合；也可使用交叉验证结果。' },
      { title: '选择应对方式', detail: '考虑增加数据、移除无关或高度冗余特征，或在目标函数中加入正则项。' },
      { title: '从较小 λ 开始', detail: '使用验证集或交叉验证比较多个非负 λ，而不是默认越大越好；测试集不参与反复调参。' },
...
print(penalty)`,
    ...
    misconceptions: [
      { wrong: '训练误差越小，模型一定越好。', why: '模型可能记住训练噪声，使验证误差反而升高。', correct: '结合训练误差与验证误差诊断拟合状态。' },
      { wrong: '可以反复查看测试集结果来选择 λ。', why: '这会让模型选择过程间接适应测试集，削弱最终评估的客观性。', correct: '使用验证集或交叉验证选择 λ，模型确定后再用测试集进行一次最终评估。' },
      { wrong: 'L2 正则化会自动删除所有无关特征。', why: 'L2 正则化通常抑制权重规模，但不保证权重精确变为 0。', correct: '它对较大权重施加更强惩罚，不是硬删除特征的规则。' },
    ],
    recap: ['训练集用于拟合模型参数；验证集用于模型比较、超参数选择和过拟合诊断；测试集只用于模型确定后的最终评估。', '欠拟合通常与高偏差相关，过拟合通常与高方差相关，但不是严格等价。', '概率论中的随机变量方差描述随机变量的波动；机器学习中的模型方差描述模型对训练样本变化的敏感程度，二者不是同一个定义。', 'J_reg 由 J_data 与 L2 正则项共同组成。', 'L2 正则化通常抑制权重规模，最终参数仍受数据拟合项影响。', '本课程约定偏置 b 不正则化。'],
  ...
  },
  {
    ...
    videoRange: '视频 35—36',
    summary: 'L2 正则化把数据拟合项与权重惩罚组成完整目标函数，并在权重梯度中增加 (λ/m)w；线性回归与逻辑回归共享这一正则项结构。',
    problem: '目标函数加入 L2 正则项后，参数更新也应包含正则项的导数；否则代码优化的仍是未正则化的数据拟合项。',
    intuition: [
      '无论线性回归还是逻辑回归，L2 正则项对 w_j 的导数都是 (λ/m)w_j，因此两者都在数据梯度上增加同一形式的正则化梯度。',
      '在本课程约定下，偏置 b 没有进入正则项，所以它的梯度不含 λb/m。实现时应显式区分 weights 与 bias。',
      '在当前使用普通梯度下降和 L2 正则项的设置下，更新公式可以改写成权重衰减形式。当 0<αλ/m<1 时，只看正则项作用，衰减乘子会在不改变符号的情况下缩小原权重的绝对值。',
      '完整更新还包含数据拟合项的梯度，因此一次更新后权重的绝对值不一定减小；“权重衰减”描述的是正则项在普通梯度下降更新中的作用。',
      '两种模型的正则项相同，但数据拟合部分不同：线性回归使用实数预测值和平方误差代价，逻辑回归使用 Sigmoid 概率估计和二元交叉熵代价。',
    ],
    formulas: [
      { label: '完整正则化目标函数', formula: 'J_{\\mathrm{reg}}(\\mathbf w,b)=J_{\\mathrm{data}}(\\mathbf w,b)+\\frac{\\lambda}{2m}\\sum_{j=1}^{n}w_j^2', explanation: '数据拟合项与 L2 正则项共同组成梯度下降实际优化的目标函数。', symbols: [{ symbol: 'J_{\\mathrm{data}}', meaning: '线性或逻辑回归各自的数据拟合代价' }, { symbol: 'J_{\\mathrm{reg}}', meaning: '完整正则化目标函数' }, { symbol: '\\lambda\\ge0', meaning: '正则化强度超参数' }] },
      { label: '正则化权重梯度', formula: '\\frac{\\partial J_{\\mathrm{reg}}}{\\partial w_j}=\\frac{\\partial J_{\\mathrm{data}}}{\\partial w_j}+\\frac{\\lambda}{m}w_j', explanation: '两种模型共享正则化梯度 λw_j/m，但各自的数据梯度不同。', symbols: [{ symbol: '\\partial J_{\\mathrm{data}}/\\partial w_j', meaning: '数据拟合项对第 j 个权重的梯度' }, { symbol: '\\lambda w_j/m', meaning: 'L2 正则项带来的附加梯度' }] },
      ...
      { label: '权重衰减形式', formula: 'w_{j,\\text{new}}=\\left(1-\\alpha\\frac{\\lambda}{m}\\right)w_j-\\alpha\\frac{\\partial J_{\\mathrm{data}}}{\\partial w_j}', explanation: '当 0<αλ/m<1 时，乘子位于 0 与 1 之间；只看正则项作用时，它会在不改变符号的情况下缩小原权重的绝对值。完整更新还包含数据拟合项的梯度，因此更新后权重的绝对值不一定减小。', symbols: [{ symbol: '\\alpha', meaning: '学习率超参数' }, { symbol: '1-\\alpha\\lambda/m', meaning: '满足 0<αλ/m<1 时位于 0 与 1 之间的衰减乘子' }] },
    ],
    algorithm: [
      { title: '计算模型输出', detail: '线性回归输出实数预测值；逻辑回归先计算线性得分，再经 Sigmoid 函数得到概率估计。' },
...
print(w_new)`,
    ...
    misconceptions: [
      ...
      { wrong: '加入 L2 正则化以后，每一次更新都会让每个权重的绝对值减小。', why: '数据梯度可能与正则化梯度方向相反，而且衰减乘子的具体性质还取决于学习率、正则化强度与样本数。', correct: '正则项增加 (λ/m)w_j 这一梯度分量；最终更新方向由数据梯度与正则化梯度共同决定。' },
      { wrong: '偏置在所有正则化约定下都必须加 λb/m。', why: '本课程定义的 L2 正则项不包含 b。', correct: '在当前课程约定下，代码把权重与偏置分开处理。' },
    ],
    recap: ['J_reg 由 J_data 与 L2 正则项组成。', '权重梯度增加 λw_j/m；在本课程约定下，偏置梯度不含正则项。', '当 0<αλ/m<1 时，仅考虑正则项，衰减乘子会缩小原权重绝对值且不改变符号。', '完整更新包含数据梯度，因此单次更新后权重绝对值不保证减小。', '学习进度已到视频 36，下一主题尚未展开。'],
  },
]
