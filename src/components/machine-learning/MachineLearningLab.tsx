...
function LearningRateLab() {
  ...
  const status = last.cost > history[0].cost * 2 ? '代价函数值上升：步长过大，更新跨过低点。' : alpha < 0.05 ? '代价函数值稳定下降，但移动较慢。' : '代价函数值下降较快，路径保持稳定。'
...
function FeatureScalingLab() {
  ...
  const sampleIndex = 4
  const formulas = { raw: '保持原始数值', chosen: 'x/s（本例 s 取各列最大绝对值）', mean: '(x-μ)/(max-min)', zscore: '(x-μ)/σ' }
  useEffect(() => {
    if (!playing) return
...
function ConvergenceLearningRateLab() {
  ...
  return <LabShell title="收敛曲线与学习率诊断" note="同一数据、同一起点运行 80 次更新；曲线和停止条件全部基于真实代价函数值。">
...
function LogisticRegressionLab() {
  ...
  return <LabShell title="Sigmoid 函数（逻辑函数）与决策边界" note="调整 w、b 和待预测输入；正类概率估计曲线、0.5 阈值与预测类别实时同步。">
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]">
