...
function SupervisedToggleLab() {
  ...
  return <LabShell title="有标签 / 无标签" note="切换同一批数据，观察任务目标怎样改变。">
    <div className="flex flex-wrap gap-2" role="group" aria-label="标签状态">
      {[true, false].map((value) => <button key={String(value)} type="button" aria-pressed={labeled === value} onClick={() => setLabeled(value)} className={`min-h-11 rounded-full px-4 text-[14px] font-semibold ${labeled === value ? 'bg-blue text-white' : 'bg-black/[0.045] text-black/60'}`}>{value ? '有标签' : '无标签'}</button>)}
    </div>
    <div className="mt-5 overflow-x-auto"><table className="w-full min-w-[420px] border-collapse text-left text-[14px]"><thead><tr className="border-b border-black/10 text-black/50"><th className="px-3 py-3">面积</th><th className="px-3 py-3">卧室数</th>{labeled && <th className="px-3 py-3 text-blue">价格（标签）</th>}</tr></thead><tbody>{rows.map((row) => <tr key={row[0]} className="border-b border-black/[0.06]"><td className="px-3 py-3">{row[0]}</td><td className="px-3 py-3">{row[1]}</td>{labeled && <td className="px-3 py-3 font-medium text-blue">{row[2]}</td>}</tr>)}</tbody></table></div>
    <p className="mt-5 rounded-[16px] bg-blue/[0.055] p-4 text-[15px] leading-7 text-black/60">{labeled ? '任务：学习面积、卧室数到价格的映射，属于监督回归。' : '任务：只能根据面积和卧室数寻找相似房屋群，不能直接学习价格预测。'}</p>
  </LabShell>
}
...
function CostCurve({ w }: { w: number }) {
  ...
  return <svg role="img" aria-label={`成本曲线，当前 w=${w.toFixed(2)}`} viewBox="0 0 530 305" className="h-auto w-full min-w-[470px]">
...
function CostFunctionLab() {
  const [w, setW] = useState(80)
  return <LabShell title="模型空间 ↔ 成本空间" note="同一个 w 同时决定左侧直线和右侧成本曲线上的位置。">
...
function LearningRateLab() {
  ...
  const status = last.cost > history[0].cost * 2 ? '成本上升：步长过大，更新跨过低点。' : alpha < 0.05 ? '成本稳定下降，但移动较慢。' : '成本下降较快，路径保持稳定。'
...
function ConvergenceLearningRateLab() {
  ...
  const status = diverged
    ? '成本快速上升：当前学习率过大。'
    : firstConverged >= 0
      ? `第 ${firstConverged + 1} 次更新后首次满足 |ΔJ|≤ε。`
      : '80 次更新内尚未达到阈值；结合曲线判断是下降缓慢还是步长不合适。'

  return <LabShell title="收敛曲线与学习率诊断" note="同一数据、同一起点运行 80 次更新；曲线和收敛判断全部来自真实成本。">
  ...
  return <LabShell title="从原始特征到多项式特征" note="切换次数，观察特征数量、拟合曲线和训练误差如何变化。">
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]">
      ...
      <div className="space-y-4">
        ...
        <p className="rounded-[15px] bg-blue/[0.055] p-4 text-[13px] leading-6 text-black/60">模型对 {`[x, x²${degree >= 3 ? ', x³' : ''}]`} 仍是线性的；弯曲来自特征变换，而不是换成另一套成本函数。</p>
      </div>
    </div>
  </LabShell>
}
...
function LogisticRegressionLab() {
  ...
  return <LabShell title="Sigmoid 与决策边界" note="调整 w、b 和待预测输入；概率曲线、0.5 阈值与分类结果实时同步。">
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]">
      <div className="min-w-0"><svg role="img" aria-label={`逻辑回归概率曲线，当前决策边界 x=${Number.isFinite(boundary) ? boundary.toFixed(2) : '未定义'}`} viewBox="0 0 530 305" className="h-auto w-full"><g stroke="rgba(29,29,31,.08)">{[0, 0.5, 1].map((p) => <line key={p} x1="48" x2="488" y1={sy(p)} y2={sy(p)} />)}</g><line x1="48" x2="488" y1="260" y2="260" stroke="rgba(29,29,31,.24)" /><line x1="48" x2="48" y1="60" y2="260" stroke="rgba(29,29,31,.24)" /><line x1="48" x2="488" y1={sy(0.5)} y2={sy(0.5)} stroke="#ff7a45" strokeWidth="2" strokeDasharray="6 6" /><path d={curve} fill="none" stroke="#0071e3" strokeWidth="4" strokeLinecap="round" />{Number.isFinite(boundary) && boundary >= -4 && boundary <= 4 && <line x1={sx(boundary)} x2={sx(boundary)} y1="55" y2="260" stroke="#8b5cf6" strokeWidth="3" strokeDasharray="7 6" />}{samples.map((sample) => <circle key={sample.x} cx={sx(sample.x)} cy={sy(sample.y)} r="6" fill={sample.y ? '#34c759' : '#ff453a'} stroke="white" strokeWidth="2" />)}<circle cx={sx(probe)} cy={sy(probability)} r="7" fill="white" stroke="#1d1d1f" strokeWidth="3" /><text x="267" y="297" textAnchor="middle" fill="rgba(29,29,31,.55)" fontSize="13">特征 x</text><text x="14" y="35" fill="rgba(29,29,31,.55)" fontSize="13">概率</text></svg></div>
      <div className="space-y-5"><Slider label="权重 w" value={w} min={0.2} max={3} step={0.1} onChange={setW} /><Slider label="偏置 b" value={b} min={-3} max={3} step={0.1} onChange={setB} /><Slider label="待预测输入 x" value={probe} min={-4} max={4} step={0.1} onChange={setProbe} /><Metric label="P(y=1|x)" value={probability.toFixed(4)} /><Metric label="预测类别" value={probability >= 0.5 ? '1' : '0'} /><Metric label="决策边界" value={Number.isFinite(boundary) ? `x=${boundary.toFixed(3)}` : '未定义'} /></div>
    </div>
  </LabShell>
}
...
function LogisticCostLab() {
  ...
  return <LabShell title="单样本逻辑损失" note="切换真实标签并调整预测概率，观察自信但错误的预测为什么受到巨大惩罚。"><div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]"><div className="min-w-0"><svg role="img" aria-label={`真实标签 ${label} 时的逻辑损失曲线，当前损失 ${loss.toFixed(4)}`} viewBox="0 0 530 305" className="h-auto w-full"><line x1="48" x2="488" y1="260" y2="260" stroke="rgba(29,29,31,.24)" /><line x1="48" x2="48" y1="60" y2="260" stroke="rgba(29,29,31,.24)" /><path d={curve} fill="none" stroke="#8b5cf6" strokeWidth="4" strokeLinecap="round" /><circle cx={sx(probability)} cy={sy(loss)} r="7" fill="white" stroke="#ff7a45" strokeWidth="4" /><text x="267" y="297" textAnchor="middle" fill="rgba(29,29,31,.55)" fontSize="13">预测概率 p</text><text x="12" y="35" fill="rgba(29,29,31,.55)" fontSize="13">损失</text></svg></div><div className="space-y-5"><div className="grid grid-cols-2 gap-2" role="group" aria-label="真实标签">{([0, 1] as const).map((value) => <button key={value} type="button" aria-pressed={label === value} onClick={() => setLabel(value)} className={`min-h-11 rounded-full text-[13px] font-semibold ${label === value ? 'bg-blue text-white' : 'bg-black/[0.045] text-black/60'}`}>真实 y={value}</button>)}</div><Slider label="预测为 1 的概率 p" value={probability} min={0.01} max={0.99} step={0.01} onChange={setProbability} /><Metric label="当前损失" value={loss.toFixed(4)} /><p className="rounded-[15px] bg-violet-500/[0.06] p-4 text-[13px] leading-6 text-black/60">{label === 1 ? 'y=1 时，希望 p 接近 1；p 越接近 0，−log(p) 越大。' : 'y=0 时，希望 p 接近 0；p 越接近 1，−log(1−p) 越大。'}</p></div></div></LabShell>
}
