...
function BoundaryInput({ label, value, onChange, discrete }: { label: 'a' | 'b'; value: number; onChange: (value: number) => void; discrete: boolean }) {
  return <label className="block min-w-0">
    <span className="mb-2 block text-xs font-semibold text-black/48">边界 {label}</span>
    <input
      type="number"
      inputMode="decimal"
      step={discrete ? 1 : 'any'}
      ...
      className="w-full rounded-xl border border-black/[0.09] bg-white/75 px-3 py-2.5 font-mono text-[15px] outline-none transition focus:border-blue/50 focus:ring-4 focus:ring-blue/10"
...
export default function DistributionCalculatorPanel({ slug, model, params, color, state, onChange }: Props) {
  ...
  const requestErrors = state.tool === 'probability' ? validateProbabilityRequest(model, request) : []
  ...
  const probabilityResult = errors.length === 0 && state.tool === 'probability' ? Math.min(1, Math.max(0, calculateProbability(model, params, request))) : Number.NaN
  ...
  const outsideSupport = state.tool === 'probability' && relevantValues.some((value) => Number.isFinite(value) && (value < support.min || value > support.max))
  ...
  const setMiddle = (probability: number) => {
    ...
    update({ tool: 'probability', mode: 'interval', a: model.quantile(tail, params), b: model.quantile(1 - tail, params) })
  }
  const setRightTail = (probability: number) => update({ tool: 'probability', mode: 'right', a: model.quantile(1 - probability, params) })
  ...
  return <div className="border-t border-black/[0.06] bg-white/25 p-5 sm:p-7 lg:p-8">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <p className="mt-1.5 text-sm leading-6 text-black/45">结果来自分布的 CDF/PMF 数学实现，不按图形像素估算。</p>
      </div>
      <button type="button" onClick={reset} className="liquid-control inline-flex self-start items-center gap-1.5 rounded-full border px-3.5 py-2 text-xs font-semibold text-black/52 transition focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue/15"><RotateCcw size={13}/>重置计算</button>
    </div>
    ...
    <div className="mt-6 grid gap-5 xl:grid-cols-[1.12fr_.88fr]">
      <div className="min-w-0 rounded-[20px] border border-black/[0.06] bg-white/55 p-4 sm:p-5">
        <div className="grid grid-cols-2 gap-2 rounded-2xl bg-black/[0.035] p-1" role="group" aria-label="计算工具">
          {[{ value: 'probability', label: '概率计算' }, { value: 'quantile', label: '分位点反查' }] .map((option) => <button key={option.value} type="button" aria-pressed={state.tool === option.value} onClick={() => update({ tool: option.value as DistributionCalculationState['tool'] })} className={`rounded-xl px-3 py-2.5 text-sm font-semibold transition focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue/15 ${state.tool === option.value ? 'bg-white text-black/80 shadow-sm' : 'text-black/42 hover:text-black/65'}`}>{option.label}</button>)}
        </div>

        {state.tool === 'probability' ? <>
          <div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-4" role="group" aria-label="概率计算模式">
            {modeOptions.map((option) => <button key={option.value} type="button" aria-pressed={state.mode === option.value} onClick={() => update({ mode: option.value })} className={`rounded-xl border px-2.5 py-2.5 text-left transition focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue/15 ${state.mode === option.value ? 'border-transparent text-white shadow-sm' : 'border-black/[0.07] bg-white/55 text-black/52 hover:bg-white'}`} style={state.mode === option.value ? { backgroundColor: color } : undefined}><span className="block text-sm font-semibold">{option.label}</span><span className={`mt-0.5 block text-[10px] ${state.mode === option.value ? 'text-white/72' : 'text-black/35'}`}>{option.hint}</span></button>)}
          </div>
          ...
          <label className="mt-5 block">
            <span className="mb-2 block text-xs font-semibold text-black/48">累计概率 p（0 &lt; p &lt; 1）</span>
            <input type="number" min="0.000001" max="0.999999" step="0.001" value={Number.isFinite(state.probability) ? state.probability : ''} onChange={(event) => update({ probability: event.currentTarget.value === '' ? Number.NaN : event.currentTarget.valueAsNumber })} className="w-full rounded-xl border border-black/[0.09] bg-white/75 px-3 py-2.5 font-mono text-[15px] outline-none transition focus:border-blue/50 focus:ring-4 focus:ring-blue/10" aria-label="分位点累计概率 p"/>
          </label>
          <div className="mt-3 flex flex-wrap gap-2" aria-label="常用累计概率">
            {[0.025, 0.05, 0.5, 0.95, 0.975].map((probability) => <button key={probability} type="button" onClick={() => update({ probability })} className="rounded-full border border-black/[0.07] bg-white/65 px-3 py-1.5 font-mono text-xs text-black/55 transition hover:border-black/[0.13] hover:bg-white focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue/15">{probability}</button>)}
          </div>
        </>}

        <div className="mt-5 border-t border-black/[0.06] pt-4">
          <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-black/32">教学预设</p>
          <div className="mt-2 flex flex-wrap gap-2">
            {slug === 'normal' && <button type="button" onClick={() => update({ tool: 'probability', mode: 'interval', a: params.mean - params.sigma, b: params.mean + params.sigma })} className="rounded-full bg-black/[0.045] px-3 py-1.5 text-xs font-medium text-black/52 hover:bg-black/[0.07] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue/15">μ ± σ</button>}
            <button type="button" onClick={() => setMiddle(0.95)} className="rounded-full bg-black/[0.045] px-3 py-1.5 text-xs font-medium text-black/52 hover:bg-black/[0.07] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue/15">中间 95%</button>
            <button type="button" onClick={() => setRightTail(0.05)} className="rounded-full bg-black/[0.045] px-3 py-1.5 text-xs font-medium text-black/52 hover:bg-black/[0.07] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue/15">右侧 5%</button>
          </div>
        </div>
      </div>
      ...
      <div className="min-w-0 rounded-[20px] p-5 text-white shadow-sm" style={{ background: `linear-gradient(145deg, ${color}, color-mix(in srgb, ${color} 76%, #111))` }} aria-live="polite">
        <div className="flex items-start justify-between gap-3">
          <div><p className="text-[11px] font-semibold uppercase tracking-[0.13em] text-white/62">{state.tool === 'probability' ? 'Probability' : 'Quantile'}</p><p className="mt-2 text-sm leading-6 text-white/78">{state.tool === 'probability' ? describeProbabilityMode(state.mode, state.a, state.b) : `q(${numberLabel(state.probability, 6)}) = min{x : F(x) ≥ p}`}</p></div>
        ...
        {errors.length === 0 ? <>
          ...
          <div className="mt-5 flex items-center gap-2 text-xs text-white/72"><CheckCircle2 size={15}/>{state.tool === 'probability' ? model.kind === 'continuous' ? '由 CDF 差值计算曲线下面积' : '按整数支持集精确累加 PMF' : model.kind === 'discrete' ? '采用最小满足 F(x) ≥ p 的整数' : '由解析反函数或稳定数值近似计算'}</div>
